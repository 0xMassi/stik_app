...
- Coordinated native and JavaScript dependency updates keep Tauri plugins build-compatible. Fixed-format regression tests protect existing encrypted notes and compressed Apple Notes imports across crypto, protobuf, and compression upgrades. Intel CI now exercises the release action without signing or publishing.
- Updated Markdown parsing, Base64 encoding, directory lookup, and bundled SQLite dependencies. Compatibility tests cover clipboard Markdown extensions and read-only Apple Notes listing/import across four synthetic schema variants. Removed an unused direct Core Graphics dependency; no SIMD or SQLite speedup is claimed from these version updates alone.
...
- Shortcut bindings use physical key positions, but some displayed shortcut labels still assume QWERTY. Layout-aware labels and live AZERTY/layout-switch verification remain tracked in [#96](https://github.com/0xMassi/stik_app/issues/96).
...
[Unreleased]: https://github.com/0xMassi/stik_app/compare/v0.9.0...HEAD
[0.9.0]: https://github.com/0xMassi/stik_app/compare/v0.8.0...v0.9.0
